# Strike Ruler Kalshi Bot v0.9.2

Production-only Kalshi KXBTC15M bot. It contains no demo or paper mode.

Market-making execution has been removed. Only `EXECUTION_STRATEGY=strike_ruler`
is supported for live trading. A stale `market_making` setting cannot silently
switch to another strategy. `TRADING_ENABLED=false` keeps all order routing off.

Existing `/data/state.json` and `/data/trades.csv` are preserved, including the
historical MM ledger. Strike Ruler does not take over a market recorded in that
ledger. Removing the executor does not sell existing positions.

Run `python -m pytest -q` for offline verification. Tests use simulated clients
and do not authenticate or send trades. `python bot.py --check` remains a
read-only production access check.
...
8. Before enabling trading, attach a persistent volume to the service at `/data`.
   ...
   ledger to bypass a budget or reconciliation block. The bot waits for state restoration
